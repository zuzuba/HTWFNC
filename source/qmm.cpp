...
void qmm_trick_blocking_AVX(float l_scale, float r_scale, float result_scale, uint4x4_t l_offset, uint4x4_t r_offset, 
	uint4x4_t result_offset, uint4x4_t* l_int_mat, uint4x4_t* r_int_mat, uint4x4_t* result_int_mat, 
	int n, int k, int m){
	...
	round_saturation_AVX(acc,result_int_mat,l_scale,r_scale,result_scale,result_offset,n,m);
	free(term2);
	free(term3);
	free(acc);
}