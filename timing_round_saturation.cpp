...
typedef void (*function_round_saturation) (int16_t* ,uint4x4_t* ,float , float , float ,uint4x4_t , int , int );


double perf_test(function_round_saturation f, char *desc,int dim);
...
void add_function(function_round_saturation f, char *name, int flops);
int validation(function_round_saturation f);


/* Global vars, used to keep track of student functions */

function_round_saturation userFuncs[MAX_FUNCS];
...
//int test_qmm()
{
	printf("------Timing function round saturation-----\n");
	int verbosity = 2;
    float cycles,perf;
...
double perf_test(function_round_saturation f, char *desc,int n)
{
	double cycles = 0.;
	double perf = 0.0;
	long num_runs = 100;
	...
	n=n/2;
	qmm_kernel_naive(lhs_q,rhs_q,l_offset,r_offset,acc,n,n,n);
	...
	do {
		...
		for (size_t i = 0; i < num_runs; i++) {
			f(acc,result_q,lhs_scale,rhs_scale,lhs_scale,l_offset,n,n);		
		}
		end = stop_tsc(start);
